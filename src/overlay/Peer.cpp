...
static std::string
msgSummary(SuperNodeMessage const& msg)
{
    switch (msg.type())
    {
    case ERROR_MSG:
...
void
Peer::sendMessage(SuperNodeMessage const& msg)
{
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            << "("
...
void
Peer::recvMessage(SuperNodeMessage const& supernodeMsg)
{
    if (shouldAbort())
    {
        return;
    ...
    if (Logging::logTrace("Overlay"))
        CLOG(TRACE, "Overlay")
            ...
            << mApp.getConfig().toShortString(
                   mApp.getConfig().NODE_SEED.getPublicKey())
            << ") recv: " << msgSummary(supernodeMsg)
            << " from:" << mApp.getConfig().toShortString(mPeerID);

    if (!isAuthenticated() && (supernodeMsg.type() != HELLO) &&
        (supernodeMsg.type() != AUTH) && (supernodeMsg.type() != ERROR_MSG))
    {
        CLOG(WARNING, "Overlay")
            << "recv: " << supernodeMsg.type() << " before completed handshake";
        mDropInRecvMessageUnauthMeter.Mark();
        drop();
        return;
    }

    assert(isAuthenticated() || supernodeMsg.type() == HELLO ||
           supernodeMsg.type() == AUTH || supernodeMsg.type() == ERROR_MSG);

    switch (supernodeMsg.type())
    {
    case ERROR_MSG:
    {
        auto t = mRecvErrorTimer.TimeScope();
        recvError(supernodeMsg);
    }
    break;
    ...
    case HELLO:
    {
        ...
        this->recvHello(supernodeMsg.hello());
    }
    break;
    ...
    case GET_SCP_QUORUMSET:
    {
        ...
        recvGetSCPQuorumSet(supernodeMsg);
    }
    break;
...
void
Peer::recvDontHave(SuperNodeMessage const& msg)
{
...
void
Peer::recvGetTxSet(SuperNodeMessage const& msg)
{
    auto self = shared_from_this();
...
void
Peer::recvTxSet(SuperNodeMessage const& msg)
{
...
void
Peer::recvTransaction(SuperNodeMessage const& msg)
{
...
void
Peer::recvGetSCPQuorumSet(SuperNodeMessage const& msg)
{
...
void
Peer::recvSCPQuorumSet(SuperNodeMessage const& msg)
{
...
void
Peer::recvSCPMessage(SuperNodeMessage const& msg)
{
...
void
Peer::recvGetSCPState(SuperNodeMessage const& msg)
{
...
void
Peer::recvError(SuperNodeMessage const& msg)
{
...
void
Peer::recvAuth(SuperNodeMessage const& msg)
{
    if (mState != GOT_HELLO)
    {
...
void
Peer::recvGetPeers(SuperNodeMessage const& msg)
{
    sendPeers();
}

void
Peer::recvPeers(SuperNodeMessage const& msg)
{
