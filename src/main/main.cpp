...
#include "main/SuperNodeCoreVersion.h"
...
static const struct option supernode_core_options[] = {
...
usage(int err = 1)
{
    ...
    os << "usage: supernode-core [OPTIONS]\n"
          ...
          "      --forcescp           Next time supernode-core is run, SCP will "
          "start "
          ...
          "signtxn\n"
          "                           (Default is SUPERNODE_NETWORK_ID "
        ...
        Json::Value& catchupInfo)
{
    ...
    catch (std::invalid_argument const&)
    {
        ...
        LOG(INFO) << "* If you really want to catchup to " << to
                  << " run supernode-core with --newdb parameter.";
        LOG(INFO) << "*";
        return 2;
    }
...
{
    LOG(INFO) << "Starting supernode-core " << SUPERNODE_CORE_VERSION;
...
main(int argc, char* const* argv)
{
    ...
    std::string cfgFile("supernode-core.cfg");
    std::string command;
    ...
    int opt;
    while ((opt = getopt_long_only(argc, argv, "c:", supernode_core_options,
                                   nullptr)) != -1)
    {
        switch (opt)
        {
        ...
        case OPT_VERSION:
            std::cout << SUPERNODE_CORE_VERSION << std::endl;
            return 0;
        case OPT_HELP:
        default:
            usage(0);
