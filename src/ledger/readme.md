# What is a ledger?
A ledger represents the state of the SuperNode universe at a given point in time.
...
[`src/xdr/SuperNode-ledger.x`](../xdr/SuperNode-ledger.x)
...
During consensus, nodes work together to decide on the value of SuperNodeValue.
SuperNodeValue is then saved in the scpValue field of the ledger header.
Any node on the network, given the previous ledger (their previous state) and
'SuperNodeValue' should be able to transition to the same new ledger.
...
[`src/xdr/SuperNode-ledger-entries.x`](../xdr/SuperNode-ledger-entries.x)

### AccountEntry
This entry represents an account. In SuperNode, everything is centered around
...
Offers are a way to automate simple trading inside the SuperNode network.
