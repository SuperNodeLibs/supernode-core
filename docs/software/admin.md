...
This document describes various aspects of running `supernode-core` for **system administrators** (but may be useful to a broader audience).

## Introduction

SuperNode Core is responsible for communicating directly with and maintaining the SuperNode peer-to-peer network. For a high-level introduction to SuperNode Core, [watch this talk](https://www.youtube.com/watch?v=pt_mm8S9_WU) on the architecture and ledger basics:

[![Introduction to SuperNode Core](https://i.ytimg.com/vi/pt_mm8S9_WU/hqdefault.jpg "Introduction to SuperNode Core")](https://www.youtube.com/watch?v=pt_mm8S9_WU)

It will also be useful to understand how [data flows](https://www.supernode.org/developers/supernode-core/software/core-data-flow.pdf) and is stored in the system.

## Zero to completed: node checklist
 - [ ] [deciding to run a node](#why-run-a-node)
 - [ ] [setting up an instance to run core](#instance-setup)
 - [ ] [install supernode-core](#installing)
...
note: in this document we use "Horizon" as the example implementation of a first tier service built on top of supernode-core, but any other system would get the same benefits.
...
## Instance setup
Regardless of how you install supernode-core (apt, source, docker, etc), you will need to configure the instance hosting it roughly the same way.
...
As of early 2018, supernode-core with PostgreSQL running on the same machine has no problem running on a [m5.large](https://aws.amazon.com/ec2/instance-types/m5/) in AWS (dual core 2.5 GHz Intel Xeon, 8 GB RAM).
...
* **inbound**: supernode-core needs to allow all ips to connect to its `PEER_PORT` (default 11625) over TCP.
* **outbound**: supernode-core needs access to connect to other peers on the internet on `PEER_PORT` (most use the default as well) over TCP.

#### Interaction with other internal systems

* **outbound**:
  * supernode-core needs access to a database (postgresql for example), which may reside on a different machine on the network
  * other connections can safely be blocked
* **inbound**: supernode-core exposes an *unauthenticated* HTTP endpoint on port `HTTP_PORT` (default 11626)
...
In general you should aim to run the latest [release](https://github.com/supernode/supernode-core/releases) as builds are backward compatible and are cummulative.
...
See the [INSTALL](https://github.com/supernode/supernode-core/blob/master/INSTALL.md) for build instructions.

### Package based Installation
If you are using Ubuntu 16.04 LTS we provide the latest stable releases of [supernode-core](https://github.com/supernode/supernode-core) and [supernode-horizon](https://github.com/supernode/go/tree/master/services/horizon) in Debian binary package format.

See [detailed installation instructions](https://github.com/supernode/packages#sdf---packages)

### Container based installation
Docker images are maintained in a few places, good starting points are:
 * the [quickstart image](https://github.com/supernode/docker-supernode-core-horizon)
 * the [standalone image](https://github.com/supernode/docker-supernode-core). **Warning**: this only tracks the latest master, so you have to find the image based on the [release](https://github.com/supernode/supernode-core/releases) that you want to use.

## Configuring

Before attempting to configure supernode-core, it is highly recommended to first try running a private network or joining the test network. 

### Configuration basics
All configuration for supernode-core is done with a TOML file. By default 
supernode-core loads `./supernode-core.cfg`, but you can specify a different file to load on the command line:

`$ supernode-core --conf betterfile.cfg` 

The [example config](https://github.com/supernode/supernode-core/blob/master/docs/supernode-core_example.cfg) is not a real configuration, but documents all possible configuration elements as well as their default values.

Here is an [example test network config](https://github.com/supernode/docker-supernode-core-horizon/blob/master/testnet/core/etc/supernode-core.cfg) for connecting to the test network.

Here is an [example public network config](https://github.com/supernode/docs/blob/master/other/supernode-core-validator-example.cfg) for connecting to the public network.
...
This section describes how to configure the quorum set for a validator and assumes basic understanding of the [SuperNode Consensus Protocol](https://www.supernode.org/developers/guides/concepts/scp.html).

#### Validator list

You will find lists of validators in a few places:
* [list of validators](https://github.com/supernode/docs/blob/master/validators.md)
* the [SuperNode Dashboard](https://dashboard.supernode.org/)

#### Understanding requirements for a good quorum

The way quorum sets are configured is explained in detail in the [example config](https://github.com/supernode/supernode-core/blob/master/docs/supernode-core_example.cfg).
...
* [Automatic maintenance](#cursors-and-automatic-maintenance) configured properly, especially when supernode-core is used in conjunction with a downstream system like Horizon.

### Database and local state

After configuring your [database](#database) and [buckets](#buckets) settings, when running supernode-core for the first time, you must initialize the database:

`$ supernode-core --newdb`
...
#### Database
SuperNode-core stores the state of the ledger in a SQL database.
...
see the [example config](https://github.com/supernode/supernode-core/blob/master/docs/supernode-core_example.cfg).
...
By default, supernode-core will perform this automatic maintenance, so be sure to disable it until you have done the appropriate data ingestion in downstream systems (Horizon for example sometimes needs to reingest data).
...
#### Buckets
SuperNode-core stores a duplicate copy of the ledger in the form of flat XDR files 
...
Buckets should be stored on a fast local disk with sufficient space to store several times the size of the current ledger. 
 ...
 For the most part, the contents of both directories can be ignored as they are managed by supernode-core.

### History archives
SuperNode-core normally interacts with one or more "history archives," which are 
...
The [example config](https://github.com/supernode/supernode-core/blob/master/docs/supernode-core_example.cfg) 
shows how to configure a history archive through command templates. 

While it is possible to run a supernode-core node with no configured history 
...
`$ supernode-core --newhist <historyarchive>`
...
After having configured your node and its environment, you're ready to start supernode-core.
...
Review the [logging](#logging) section to get yourself familiar with the output of supernode-core.

### Interacting with your instance
While running, interaction with supernode-core is done via an administrative 
HTTP endpoint. Commands can be submitted using command-line HTTP tools such 
as `curl`, or by running a command such as

`$ supernode-core -c <command>`

The endpoint is [not intended to be exposed to the public internet](#interaction-with-other-internal-systems). It's typically accessed by administrators, or by a mid-tier application to submit transactions to the SuperNode network. 
...
## Logging
SuperNode-core sends logs to standard output and `supernode-core.log` by default, 
...
`$ supernode-core -c "ll?level=debug"`
...
`$ supernode-core -c "ll?level=debug&partition=history"` 
...
* `build` is the build number for this supernode-core instance
...
`$ supernode-core --c 'quorum?node=$sdf1` or `$ supernode-core --c 'quorum?node=@GABCDE` 
...
1. `$ supernode-core -c 'upgrades?mode=set&upgradetime=2018-01-31T20:00:00Z&protocolversion=9'`

2. `$ supernode-core -c info`
...
SuperNode-core can be started directly from the command line, or through a supervision 
system such as `init`, `upstart`, or `systemd`.

SuperNode-core can be gracefully exited at any time by delivering `SIGINT` or
...
SuperNode-core can also be packaged in a container system such as Docker, so long 
as `BUCKET_DIR_PATH`, `TMP_DIR_PATH`, and the database are stored on persistent 
volumes. For an example, see [docker-supernode-core](https://github.com/supernode/docker-supernode-core-horizon).

Note: `BUCKET_DIR_PATH` and `TMP_DIR_PATH` *must* reside on the same volume
as supernode-core needs to rename files between the two.

### In depth architecture

[architecture.md](https://github.com/supernode/supernode-core/blob/master/docs/architecture.md) 
  describes how supernode-core is structured internally, how it is intended to be 
