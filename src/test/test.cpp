...
bool force_sqlite = (std::getenv("SUPERNODE_FORCE_SQLITE") != nullptr);
...
getTestConfig(int instanceNumber, Config::TestDbMode mode)
{
    ...
    {
        gTestRoots.emplace_back("supernode-core-test");

        std::string rootDir = gTestRoots.back().getName();
        rootDir += "/";

        cfgs[instanceNumber] = supernode::make_unique<Config>();
        Config& thisConfig = *cfgs[instanceNumber];
        thisConfig.USE_CONFIG_FOR_GENESIS = true;

        std::ostringstream sstream;

        sstream << "supernode" << instanceNumber << ".log";
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Testing supernode-core " << SUPERNODE_CORE_VERSION;
