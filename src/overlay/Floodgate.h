...
class Floodgate
{
    class FloodRecord
    {
      public:
        ...
        uint32_t mLedgerSeq;
        SuperNodeMessage mMessage;
        std::set<Peer::pointer> mPeersTold;

        FloodRecord(SuperNodeMessage const& msg, uint32_t ledger,
                    Peer::pointer peer);
    };
  ...
  public:
    ...
    bool addRecord(SuperNodeMessage const& msg, Peer::pointer fromPeer);

    void broadcast(SuperNodeMessage const& msg, bool force);
