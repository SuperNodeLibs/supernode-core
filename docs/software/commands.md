...
supernode-core can be controlled via the following commands.

## Command line options
* **--?** or **--help**: Print the available command line options and then exit..
* **--c** Send an [HTTP command](#http-commands) to an already running local instance of supernode-core and then exit. For example: 

`$ supernode-core -c info`

* **--conf FILE**: Specify a config file to use. You can use '-' and provide the config file via STDIN. *default 'supernode-core.cfg'*
...
`$ supernode-core --convertid SDQVDISRYN2JXBS7ICL7QJAEKB3HWBJFP2QECXG7GZICAHBK4UNJCWK2`
...
the database. The next time supernode-core is run, supernode-core will start 
emitting SCP messages based on its last known ledger. Without this flag supernode-core waits to hear
...
* **--newhist ARCH**:  Initialize the named history archive ARCH. ARCH should be one of the history archives you have specified in the supernode-core.cfg. This will write a `.well-known/supernode-history.json` file in the archive root.
...
* **--signtxn FILE**:  Add a digital signature to a transaction
  ...
  appears to be coming from a terminal, supernode-core disables echo.
  Note that if you do not have a SUPERNODE_NETWORK_ID environment
...
* **--sec2pub**:  Reads a secret key on standard input and outputs the
  corresponding public key.  Both keys are in SuperNode's standard
  base-32 ASCII format.
* **--netid STRING**:  The --signtxn option requires a particular
  network to sign for.  For example, the production supernode network is
  "`Public Global SuperNode Network ; September 2015`" while the test
  network is "`Test SDF Network ; September 2015`".
* **--test**: Run all the unit tests. For [further info](https://github.com/philsquared/Catch/blob/master/docs/command-line.md) on possible options for test. For example this will run just the "Herder" tests and stop after the first failure: `supernode-core --test -a [Herder]` 
* **--version**: Print version info and then exit.


## HTTP Commands
By default supernode-core listens for connections from localhost on port 11626. 
You can send commands to supernode-core via a web browser, curl, or using the --c 
...
* **setcursor**
 `/setcursor?id=ID&cursor=N`<br>
  ...
  Cursors are used by dependent services to tell supernode-core which data can be safely deleted by the instance.
