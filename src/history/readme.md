...
The `get` and `put` commands define subprocesses that `supernode-core` will
...
called `s3supernode` based on the [AWS S3](https://aws.amazon.com/s3/) storage system:

~~~~
[HISTORY.s3supernode]
get="curl http://history.supernode.org/{0} -o {1}"
put="aws s3 cp {0} s3://history.supernode.org/{1}"
~~~~

In this example, `supernode-core` will use the `curl` command to fetch history and
