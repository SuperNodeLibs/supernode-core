...
supernode-core is at the bottom of the SuperNode stack, many other services can be
 built on top of it.
...
# Integration points
## Data consumers
Many types of services can be built based on the data generated by supernode-core.

supernode-core generates two types of data that can be queried by applications.

Unless otherwise noted, data stored is `validated`, ie data is only persisted
after consensus is reached on the network.

An overview of how the data flows within supernode-core is [this data flow presentation](software/core-data-flow.pdf)
...
properly registered by updating its cursor so that supernode-core can safely
garbage collect data no longer needed.
By default supernode-core only keeps track of its internal state and will
 ...
 * additional meta data not available in history is produced by supernode-core.
 ...
 * does not depend on running supernode-core (can be built as a pure client
...
### SuperNode core database

## API to supernode-core
  supernode-core exposes a simple HTTP based API for performing various
  types of operations on supernode-core itself or on entities in the network
