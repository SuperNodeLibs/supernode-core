...
using namespace supernode::txtest;
...
CommandHandler::CommandHandler(Application& app) : mApp(app)
{
    if (mApp.getConfig().HTTP_PORT)
    {
        ...
        mServer = supernode::make_unique<http::server::server>(
...
{
    retStr = "<b>Welcome to supernode-core!</b><p>";
    ...
    retStr +=
        ...
        "Cursors are used by dependent services to tell supernode - core which "
...
CommandHandler::manualClose(std::string const& params, std::string& retStr)
{
    ...
    {
        retStr =
            "Set MANUAL_CLOSE=true in the supernode-core.cfg if you want this "
            "behavior";
    }
}
...
CommandHandler::generateLoad(std::string const& params, std::string& retStr)
{
    ...
    else
    {
        retStr = "Set ARTIFICIALLY_GENERATE_LOAD_FOR_TESTING=true in "
                 "the supernode-core.cfg if you want this behavior";
    }
}
