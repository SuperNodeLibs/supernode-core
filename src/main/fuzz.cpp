// Copyright 2015 SuperNode Development Foundation and contributors. Licensed
 ...
 *     random SuperNodeMessages. This is the mode you use to generate seed data
 *     for the external fuzzer's corpus.
 *
 *   - In --fuzz mode it reads back a file and appplies it to a pair of of
 *     supernode-cores in loopback mode, cranking the I/O loop to simulate
...
std::string
msgSummary(SuperNodeMessage const& m)
{
...
bool
tryRead(XDRInputFileStream& in, SuperNodeMessage& m)
{
    try
    {
        return in.readOne(m);
     ...
     std::vector<std::string> const& metrics)
{
    ...
    LOG(INFO) << "Fuzzing supernode-core " << SUPERNODE_CORE_VERSION;
...
genfuzz(std::string const& filename)
{
    ...
    out.open(filename);
    autocheck::generator<SuperNodeMessage> gen;
    for (size_t i = 0; i < n; ++i)
    {
        try
        {
            SuperNodeMessage m(gen(10));
            out.writeOne(m);
