...
void
HerderImpl::valueExternalized(uint64 slotIndex, SuperNodeValue const& value)
{
    // record metrics
...
HerderImpl::triggerNextLedger(uint32_t ledgerSeqToTrigger)
{
    ...
    SuperNodeValue newProposedValue(txSetHash, nextCloseTime, emptyUpgradeSteps,
                                  0);
    ...
    std::vector<TransactionFramePtr> const& applied)
{
    ...
    // propagation
    {
        ...
        {
            auto msg = tx->toSuperNodeMessage();
