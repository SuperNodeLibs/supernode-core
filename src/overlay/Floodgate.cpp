...
Floodgate::FloodRecord::FloodRecord(SuperNodeMessage const& msg, uint32_t ledger,
                                    Peer::pointer peer)
...
bool
Floodgate::addRecord(SuperNodeMessage const& msg, Peer::pointer peer)
{
    if (mShuttingDown)
    {
        return false;
...
void
Floodgate::broadcast(SuperNodeMessage const& msg, bool force)
{
    if (mShuttingDown)
    {
        return;
