...
class HerderSCPDriver : public SCPDriver
{
  public:
    struct ConsensusData
    {
        ...
        SuperNodeValue mConsensusValue;
        ConsensusData(uint64_t index, SuperNodeValue const& b)
    ...
    void restoreSCPState(uint64_t index, SuperNodeValue const& value);
    ...
    void nominate(uint64_t slotIndex, SuperNodeValue const& value,
                  TxSetFramePtr proposedSet, SuperNodeValue const& previousValue);
  ...
  private:
    ...
    validateValueHelper(uint64_t slotIndex, SuperNodeValue const& sv) const;
