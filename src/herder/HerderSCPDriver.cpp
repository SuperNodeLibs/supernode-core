...
void
HerderSCPDriver::restoreSCPState(uint64_t index, SuperNodeValue const& value)
{
...
HerderSCPDriver::getValueString(Value const& v) const
{
    ...
    try
    {
        ...
        return supernodeValueToString(b);
    }
    catch (...)
    {
        return "[:invalid:]";
                                   ...
                                   std::set<Value> const& candidates)
{
    ...
    SuperNodeValue comp(h, 0, emptyUpgradeSteps, 0);
    ...
    std::vector<SuperNodeValue> candidateValues;

    for (auto const& c : candidates)
    {
        candidateValues.emplace_back();
        SuperNodeValue& sv = candidateValues.back();
...
HerderSCPDriver::valueExternalized(uint64_t slotIndex, Value const& value)
{
    ...
    catch (...)
    {
        ...
        // therefore contain a valid SuperNodeValue.
        CLOG(ERROR, "Herder") << "HerderSCPDriver::valueExternalized"
                              << " Externalized SuperNodeValue malformed";
...
void
HerderSCPDriver::nominate(uint64_t slotIndex, SuperNodeValue const& value,
                          TxSetFramePtr proposedSet,
                          SuperNodeValue const& previousValue)
{
